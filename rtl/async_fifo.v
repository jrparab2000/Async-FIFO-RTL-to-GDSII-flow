module async_fifo #(parameter SIZE_FIFO = 8, parameter DATA_SIZE = 32) (
    ...
    mem #(.SIZE_FIFO(SIZE_FIFO), .DATA_SIZE(DATA_SIZE)) memory (.clk(wclk), .rst_n(wrst_n), .data_in(data_in), .wtr_ptr(wtr_b_ptr[SIZE-1:0]), .full(full_i), .wtr_en(wtr_en), .rd_ptr(rd_b_ptr[SIZE-1:0]), .rd_en(rd_en), .empty(empty_i), .data_out(data_out_i));
